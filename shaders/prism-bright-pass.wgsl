...
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  // The knee starts above diffuse white. Centering it on the threshold
  // would bloom the light theme's untouched background into a milky veil.
  let soft = clamp((luminance - THRESHOLD) / (2.0 * KNEE), 0.0, 1.0);
  let contribution = max(soft * soft * KNEE, luminance - THRESHOLD - KNEE);
